...
class Microblading extends CI_Controller {
	...
	public function index()
	{
		if(!$this->contenido_model->load_page(PAGINA_MANTENIMIENTO)){
			if($this->contenido_model->load_page(PAGINA_MICROBLADING)){
				$data['text_1']	= $data['text_2'] = '';
				$data['img_1'] = $data['img_2'] = '';
				$data['gallery'] = '';
				$header['navbar'] = '';
				$head['title'] = 'Anamar BL-Microblading';
				$seccion = $this->contenido_model->get_contenido(PAGINA_MICROBLADING, 1);
				if($seccion){
					$data['text_1'] = $seccion->text;
					$data['img_1'] = $seccion->background_img;
				}
				$seccion = $this->contenido_model->get_contenido(PAGINA_MICROBLADING, 2);
				if($seccion){
					$data['text_2'] = $seccion->text;
					$data['img_2'] = $seccion->background_img;
				}

				$seccion = $this->contenido_model->get_contenido(PAGINA_MICROBLADING, 3);
				if($seccion){
				...
				$this->load->view('front/microblading',$data);
