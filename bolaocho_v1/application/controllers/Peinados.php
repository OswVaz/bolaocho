...
class Peinados extends CI_Controller {
	...
	public function index()
	{
		if(!$this->contenido_model->load_page(PAGINA_MANTENIMIENTO)){
			if($this->contenido_model->load_page(PAGINA_PEINADOS)){
				$data['text_1']	= $data['text_2'] = '';
				$data['img_1'] = $data['img_2'] = '';
				$data['gallery'] = '';
				$header['navbar'] = '';
				$head['title'] = 'Anamar BL-Peinados';

				$seccion = $this->contenido_model->get_contenido(PAGINA_PEINADOS, 1);
				if($seccion){
					$data['text_1'] = $seccion->text;
					$data['img_1'] = $seccion->background_img;
				}
				$seccion = $this->contenido_model->get_contenido(PAGINA_PEINADOS, 2);
				if($seccion){
					$data['text_2'] = $seccion->text;
					$data['img_2'] = $seccion->background_img;
				}

				$seccion = $this->contenido_model->get_contenido(PAGINA_PEINADOS, 3);
				if($seccion){
				...
				$this->load->view('front/peinados',$data);
